...
local source_environments = config.source_environments or fail("'source_environments' was not defined in '.lunarconfig'")
...
for source_env_index = 1, (#source_environments) do
  ...
  local function parse_subpath(path, dot_path, name)
    if path == out_dir or path == PathUtils.join(".", out_dir) then
      return
    end
    ...
    if assert_file_ext(name, ".lunar") or assert_file_ext(name, ".lua") then
      if used_paths[file_path_dot] then
        if (assert_file_ext(used_paths[file_path_dot], ".lua") and assert_file_ext(name, ".d.lunar")) or (assert_file_ext(used_paths[file_path_dot], ".d.lunar") and assert_file_ext(name, ".lua")) then
        else
    ...
    elseif attrs and attrs.mode == "file" and (assert_file_ext(name, ".lunar") or assert_file_ext(name, ".d.lunar")) then
      ...
      parse_and_bind_source(io.open(file_path):read("*a"), file_path, file_path_dot, ((not assert_file_ext(name, ".d.lunar"))) and out_path or nil)
  ...
  for i = 1, (#env_modules) do
    ...
    if absolute_path and attrs and attrs == "file" then
      parse_and_bind_source(io.open(absolute_path):read("*a"), absolute_path, source_path_dot, nil)
    end
  end
